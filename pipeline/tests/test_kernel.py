...
def test_펼치면_모서리_노출량_급락이_완화된다():
    """중심 한 점에 몰면 셀 모서리에서 E가 급락해 λ̂가 인위적으로 치솟는다.

    모서리(450,450)는 중심에서 636m — K=0.039까지 떨어진다. 실제 가구는 셀에 퍼져
    있으므로 펼쳐서 얹으면 가까운 하위 점이 받쳐 준다.
    """
...
def test_이질적이면_m이_작고_균질하면_크다():
    """m은 '몇 가구분의 관측과 prior를 맞먹게 볼 것인가'다.

    ⚠️ 노출량이 상수면 P10=P90이라 가드레일이 모든 값을 한 점으로 클립한다.
    실제 격자는 가구수가 제각각이므로 픽스처도 분산을 줘야 의미가 있다.
    """
...
def test_w_sev를_m_추정에_섞으면_축소가_약해진다():
    """이 함정을 문서가 아니라 테스트로 잠근다.

    가중 합은 과산포라 V̂ar를 부풀려 m을 작게 만든다 → 축소가 약해져
    '한 건 터진 소규모 지역이 1등으로 튀는 것을 막는다'는 목적과 정반대가 된다.
    """
...
def test_노출이_작으면_원시_발생률이_평균쪽으로_당겨진다():
    """소표본 스파이크 억제 — 축소항의 존재 이유.

    축소는 **원시 발생률 S/E 기준**으로 봐야 한다. 같은 발생률이라도 노출이 작으면
    Z=E/(E+m)이 작아 λ̄ 쪽으로 더 당겨진다. 한 건 터진 소규모 지역이 1등으로 튀는 것을
    막는 게 이 항의 목적이다.
    """
