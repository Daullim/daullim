"""`build_scored()` 프로세스 캐시 — 중복 재계산 제거(이슈 #28) 회귀 방지."""
...
def counting_compute(monkeypatch):
    """`_compute_scored`를 호출 카운터로 대체 — `fit_sidos`도 기록해 LORO 폴드 분리 검증."""
    calls = {"n": 0, "keys": []}
...
def test_반환값을_변형해도_다음_호출이_오염되지_않는다(counting_compute):
    """호출부(`assign_order`·`relative_risk`)의 입력 copy 규약이 깨져도 캐시는 원본 보존."""
...
# ── LORO 폴드 분리 ──────────────────────────────────────────────────────
# 캐시 키에 fit_sidos 누락 시 첫 폴드 결과가 세 폴드에 재사용돼 조용히 동일값 3개 산출
