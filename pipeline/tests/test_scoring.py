...
def test_risk_level은_절대_임계값이다():
    """분위수로 하면 화면에 항상 일정 비율이 빨간색으로 남아
    '위험 가구가 줄었는가'를 볼 수 없다 — 기획서 §5-1과 어긋난다."""
...
def test_동점이면_연차가_오래된_쪽이_앞이다():
    """§2-3의 2단은 원래 만료확률인데 보급이력이 전량 NULL이라 무력화됐다 —
    같은 자리에 연차를 놓는다(만료확률의 대리, x₁과 같은 방향)."""
...
def test_연차_결측은_맨_뒤로_간다():
    """'모름'을 '가장 오래됨'으로 두면 안 된다.

    결측을 0으로 두면 실제 날짜(-738,000 근처)보다 커서 DESC 정렬에서 1등이 된다 —
    실측에서 임실군 큐 상위 5건이 전부 사용승인일 결측이었다.
    """
