...
답하는 것 — *데이터 의존 파라미터가 지역을 넘어 전이되는가.* 클래스별 m·λ̄와 V⊥ 백분위
기준을 학습 2시도에서만 적합하고, 홀드아웃 시도 건물을 그 파라미터로 채점.

답하지 **못하는** 것 — *미래 예측력.* λ̂는 건물 주변의 과거 화재로 계산하는데 라벨도 같은
화재로 만든다. 홀드아웃 시도의 화재를 λ̂ 입력에서 빼면 λ̂가 사전값으로 뭉개져 모델 자체가
사라지므로 뺄 수 없다. 즉 **λ̂ 항은 여전히 인샘플**이고, 시간 분할로만 풀리는데 화재 상품이
2023 단년이라 지금 데이터로는 불가능.

따라서 아래 수치는 **지역 간 전이 지표**이지 절대 성능이 아님.
"""
...
def labels_by_building() -> pd.Series:
    ...
    라벨은 화재 지점↔건물 최근접 매칭이라 학습 범위와 무관하다. 폴드마다 다시 붙이면
    같은 계산을 세 번 하고(수십 초), 무엇보다 **폴드에 따라 라벨이 달라 보이는 착시**를 만든다.
    """
...
def fold(holdout: str, labels: pd.Series, *, refit_beta: bool) -> dict:
    ...
    # ②③만 — λ̂를 뺀 항. **라벨을 만든 화재가 들어가지 않는 유일한 열**이라, 인샘플성에
    # 오염되지 않은 전이 신호를 여기서 읽는다.
    scored["area_rr"] = scored["area_mult"] * scored["rr_i"]

    te = scored.loc[scored["sido"] == holdout].copy()

    # 베이스라인 — 포집률 단독은 해석이 안 된다. 무엇 대비 좋은지가 있어야 읽힌다.
    scores = {
        "가구수(노출량)": "exposure",
        "②③만(λ̂ 제외)": "area_rr",
        "λ̂만": "lambda_hat",
