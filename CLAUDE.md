...
파이프라인이 `buildings` 94,240행 · `units` 243,846행을 적재했고, API 15종과
`docs/openapi.yaml`이 붙어 **화면이 실데이터로 돈다**. 점검 결과 저장·조회가 모두 붙어
현장 폼이 서버에 남고 `/records`가 그것을 되읽는다 — **목데이터는 `/demo`용 한 벌만 남았다**.
시연 지역은 **6곳**(ADR-010 개정 2, 2026-08-12) — 서울 관악·**강북**, 부산 부산진·기장, 전북 임실·**고창**.
세 시도가 모두 시군구 둘씩이라 시도마다 도시·농촌 대비가 선다.
**6지역 데이터로 화면은 아직 안 봤다** — 파이프라인·DB·BE 테스트까지만 확인.
지도 격자 밀집 표현도 **아주 옅은 코로플레스로 확정·구현**됐다(2026-08-11) — 아래 Known Gaps 참조.
...
- ~~도시 격자 위험 밀집 표시 방식 결정 대기~~ → **확정·구현됨**(2026-08-11). **아주 옅은 코로플레스** — 등급 본색을 낮은 농도로(danger 0.14 · warn 0.08 · ok 0.03) 채우고 선택은 테두리로만 표시한다. 규칙은 `frontend/DESIGN.md` § 격자 위험 코로플레스, 구현은 `frontend/src/lib/grid-layer.ts`.
- ~~동 경계 GeoJSON 미확보~~ → **확보·실장됨**(2026-08-06). 출처는 [admdongkor](https://github.com/vuski/admdongkor) `ver20250701`이고 `seed/admin-dong-boundaries.geojson`(**85개 행정동**, 2026-08-12 강북·고창 편입 후 재절단)이 정본이다. B1이 `GET /regions/boundaries`로 받아 시군구 단위로 그리고 폴리곤 클릭↔동 선택이 양방향으로 물린다. **이 파일은 사본이 3벌인 데다 `run_seed.py`가 만들지 않는다** — 시연 지역을 늘리면 다시 잘라야 하고, 안 하면 지도만 조용히 빈다. `pipeline/recut_boundaries.py`가 사본 3벌까지 한 번에 맞춘다.
