...
description: 다울림(일반주택 화재경보기 사후관리 우선순위 처방 엔진)의 "차분한 관제실(calm control-room)" 디자인 시스템 — 회백 캔버스 위 흰 서피스, 관제 블루 악센트 1색, 저채도 위험 3색(레드·앰버·그린)이 유일한 색 서사. 라벨은 Pretendard, 값은 JetBrains Mono. hairline 보더 우선, 라이트 테마 단일, 장식 없음.
...
typography: # 굵기는 400/600 두 단계만. 라벨=sans, 값=mono 경계 불변.
  display-sm:
  ...
  data-mono:
    ...
    fontSize: inherit   # 주변 sans 크기를 따르되 서체만 mono로 전환
    fontWeight: 400
    lineHeight: inherit
  data-lg:
    fontFamily: "JetBrains Mono, ui-monospace, monospace"
...
components: # 코어 8종 — 토큰 조합 정의 (구현: src/components/core/)
  ...
  risk-badge:
    ...
    content: "라벨(sans) + 수치(mono) 병기 필수 — 색 단독 금지"
    rounded: "{rounded.sm}"
    typography: "{typography.caption} + {typography.data-mono}"
  data-text:
    fontFamily: "{typography.data-mono}"
    textColor: "inherit (기본) | {colors.text-body}"
    use: "risk_score·grid_id·rx_code·좌표·건수 — 값이면 무조건 mono"
  estimate-border:
  ...
  queue-row:
    ...
    cells: "순위·점수·격자ID=mono, 주소·기준=sans"
  region-selector:
...
- 라벨은 Pretendard(sans), 값(risk_score·grid_id·rx_code·좌표·건수)은 JetBrains Mono. 이 경계가 "기능적 신뢰감"의 핵심 장치다.
...
| `{typography.data-mono}` | JetBrains Mono | 상속 | 400 | 상속 | 모든 데이터 값 |
| `{typography.data-lg}` | JetBrains Mono | 24px | 600 | 1.2 | 요약 카운터 숫자 |
| `{typography.button}` | Pretendard | 15px | 600 | 1 | 버튼 라벨 |

### 원칙 — "라벨 sans / 값 mono 경계는 절대 불변"
`risk_score`, `grid_id`, `rx_code`, 좌표, 건수, 순위 — **값이면 무조건 `DataText`(mono)로 감싼다.** 라벨·문장은 무조건 Pretendard. mono로 문장을 쓰거나 sans로 값을 쓰는 순간 시스템이 무너진다. 굵기는 400/600 두 단계만 — 500·700 금지. 디스플레이 전용 서체 도입 금지(업무 도구).
...
3. **`RiskBadge`** (`{components.risk-badge}`) — 위험/경고/양호 3구간. 틴트 배경 + line 보더 + **라벨(sans) + 수치(mono) 병기 필수형** — 수치 없는 사용은 라벨만이라도 강제.
4. **`DataText`** (`{components.data-text}`) — mono 값 표기 래퍼. `<DataText>GA-0412</DataText>`. 값이면 무조건 이걸 통과시킨다.
