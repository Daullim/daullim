...
colors: # [semantic 계층] — 컴포넌트·화면이 참조하는 유일한 계층. palette만 참조.
  ...
  bg-dark: "{palette.gray-900}"         # 유일한 다크 서피스 — 관제 하단 요약 바 한 곳(5탭 공통)
...
components: # 코어 10종 — 토큰 조합 정의 (구현: src/components/core/)
  button-primary:
  ...
  bar-row: # 분포 1줄 — 라벨 + 막대 + 수치
    track: "{colors.bg-muted}"
    fill: "위험 축은 {colors.risk-*-tint} + {colors.risk-*-line} 1px / 그 외는 {colors.accent}"
    height: 8px
    rounded: "{rounded.xs}"
    content: "라벨 + 수치 병기 필수 — 막대만으로 정보를 전달하지 않는다"
    estimated: "값이 추정·미상이면 트랙에 점선 보더 ({states.estimated})"
  stacked-bar: # 100% 비율 막대 — 세그먼트 n개 + 범례
    segments: "{colors.risk-*-tint} 채움 + {colors.risk-*-line} 1px 구분선"
    labelColor: "{colors.risk-*} 본색"
    height: 12px
    rounded: "{rounded.xs}"
    minSegment: "1px — 0건이 아닌 세그먼트는 반드시 보인다"
---
...
- **Dark** (`{colors.bg-dark}`): **유일한 다크 서피스** — `/control` 하단 요약 카운터 바 한 곳에만 허용. 관제 5탭이 이 바 하나를 공유하며 **KPI를 말하는 자리도 여기뿐이다**(탭별 KPI 스트립 금지). 다른 어디에도 다크 면을 만들지 않는다.
...
| 지도 격자 면 | 본색을 **아주 낮은 농도**로 채운다 — 아래 § 격자 위험 코로플레스 |
...
#### 격자 위험 코로플레스 (2026-08-11 확정)

B2 지도의 1km 격자는 **면을 위험 등급 본색으로 채우되 농도를 아주 낮게** 둔다. 정본은
`src/lib/grid-layer.ts`의 `gridStyles()`이며, 색은 배지·범례와 **같은 토큰**을 쓴다.

| 등급 | 토큰 | `fillOpacity` |
|---|---|---|
| danger | `--color-risk-danger` | **0.14** |
| warn | `--color-risk-warn` | **0.08** |
| ok | `--color-risk-ok` | **0.03** |
| 등급 없음 | `--color-ink` | 0.02 |

- **등급마다 농도가 다르다.** 실측(4지역 704격자) 분포가 `ok` 461 · `warn` 214 · `danger` 29라
  같은 농도로 칠하면 화면 65%가 초록으로 덮여 정작 위험 격자가 묻힌다. 안전한 쪽을 더 옅게 해
  **눈이 danger·warn에 먼저 가도록** 기울인 값이다.
- **`ok`도 0으로 두지 않는다.** `fillOpacity: 0`이면 면이 클릭 이벤트를 받지 못해 격자 선택이 죽는다.
- **선택 강조는 테두리로만 한다**(브랜드색 3px). 면을 브랜드색으로 덮으면 **선택하는 순간 그 격자의
  위험도가 사라진다** — 선택은 "리스트와 지도가 같은 것을 가리킨다"는 신호이지 위험 표현이 아니다.
- 병기 의무는 그대로다. 지도 색은 훑어보기용이고 **등급 라벨·점수는 리스트의 `RiskBadge`가 말한다** —
  색 단독으로 정보를 전달하지 않는다.

#### 분포 막대 색 규칙 (2026-08-12 확정 — 관제 분석 탭)

`BarRow`·`StackedBar`가 그리는 분포·구성·진행률의 색은 **새로 만들지 않는다.**

| 막대가 재는 것 | 색 |
|---|---|
| **위험 축** (등급 구성 · 도농×등급 · 동별 지도) | `{colors.risk-*-tint}` 채움 + `{colors.risk-*-line}` 1px 구분선 + `{colors.risk-*}` 본색 수치 라벨 |
| **위험 축이 아닌 분포** (주택유형 · 건축 연대 · 거부 사유 · 자재 구분) | `{colors.accent}` **단색 + 길이로만** 구분 |
| 추정·미상 세그먼트 (예: 사용승인일 미상) | 점선 보더 — `{states.estimated}` 문법 그대로 |

- **위험 축 막대에 본색 solid를 쓰지 않는다** — 위 표의 "넓은 면적을 본색으로 칠하지 않는다"와 같은 규칙이다.
- **카테고리 색을 만들지 않는다.** 주택유형 5종·연대 12구간에 각각 색을 주면 그 순간 위험 3색 + 악센트
  1색이라는 색 서사가 무너진다. **농도 계단도 쓰지 않는다** — 농도는 격자 코로플레스가 이미 쓰는 문법이라
  같은 화면에서 다른 뜻으로 겹친다.
- **병기 의무는 그대로다.** 모든 막대에 라벨과 수치를 붙인다 — 막대 길이만으로 정보를 전달하지 않는다.
...
- **`/control` (태블릿 1024×768 가로 = 검증 기준, ≥1280은 여유 폭 활용) — 분석형 대시보드 5탭.** 관제는 **개별 건물을 가리키지 않는다** — 건물을 짚는 순간 `/field`의 데스크톱판이 된다. 드릴다운의 종점은 **동**이고 그 아래는 `[현장모드로 전환]`이 받는다. 골격 = 상단바(56px) → **탭바** → `<Outlet />` → 하단 다크 요약 바(64px).
  - **탭 5종은 라우트다** — `/control`(관내 현황) · `/control/risk`(위험 분석) · `/control/progress`(추진 현황) · `/control/materials`(소요 물량) · `/control/report`(실적 통계). 시연 중 특정 탭 URL 직행이 되고 새로고침에 살아남는다. **shadcn Tabs가 아니라 `<nav>` + `NavLink`** — 라우트 탭은 Tabs 프리미티브가 아니라 내비게이션이라 링크만으로 접근성이 끝난다(roving tabindex·`aria-selected`를 흉내 낼 일이 없다). Iteration Guide 6의 예외가 아니라 적용 대상이 아니다.
  - **탭바 = 좌 탭 / 우 `RegionSelector`(시·도 → 시·군·구 **2단**) + `[현장모드로 전환]`.** 관할은 다섯 탭 전체의 스코프라 탭보다 상위에 둔다 — 탭마다 필터를 반복하면 전환 시 스코프가 흔들린 것처럼 보인다. **읍·면·동 단계는 두지 않는다**(동은 표·지도에서 고른다).
  - **`/control`은 지역 셀렉터·다크 바를 레이아웃 라우트가 들고 자식 탭이 `<Outlet />`으로 들어온다.** 조회 훅이 탭 전환에 언마운트되지 않아 다크 바 숫자가 깜빡이지 않는다 — 캐시 계층 없이(ADR-004 §3 v1.4) 화면 간 값을 공유하는 유일한 방법이다.
  - **KPI는 다크 요약 바 하나로만 말한다.** 탭별 KPI 스트립을 따로 두지 않는다 — 위아래로 같은 숫자가 두 번 나오고, 다크 서피스를 위로 옮기면 "하단 요약 바 한 곳" 규정과 어긋난다. 카운터 4칸(대상 세대 · 위험 등급 주택 · 예상 소요 · 점검 완료) + 우측 관할명.
  - **범례는 지도 카드 안 플로팅**(`absolute top-3 right-3`, B1·B3과 동일 배치)이다. 탭바에서 뺐다고 없애지 않는다 — 상시 노출 규칙은 그대로다.
  - **본문 세로 스크롤을 허용한다.** 분석 화면은 1024×768에서 반드시 스크롤이 생긴다. 다만 **상단바·탭바·다크 바는 고정**이고 `<Outlet />`만 스크롤한다 — 스코프(관할)와 총량(다크 바)이 언제나 보여야 한다.
  - **① 관내 현황** = 좌 지도(동 경계 코로플레스, flex-1) + 우 동별 현황 표(560px~) → 하단 신뢰도·신선도 띠(좌표 추정 건수 · 사용승인일 미상 건수 · **산출 시각**). 표 컬럼 = 동 · 대상 세대 · 위험 주택 · **배수** · 진행률, **기본 정렬은 배수(`avgRrI`) 내림차순**(위험 주택 수는 큰 동이 늘 위라 작은 동의 신호가 죽는다). 폴리곤 클릭 ↔ 표 행 선택이 양방향. **시·군·구 미선택이면 같은 표가 시군구 행으로 바뀐다**(지역 비교 — 새 UI 없음). 산출 시각은 `computed_at`이다 — 파이프라인은 월 1회 돌므로 `LastUpdated`(응답 시각)와 다른 축이며 둘을 섞지 않는다.
  - **② 위험 분석** = 등급 구성(`StackedBar`) · 도농×등급(3줄) · 주택 유형 · 건축 연대(`use_apr_day`) · 상대위험도 분포. **지도를 두지 않는다** — 같은 지도를 두 탭에 두면 탭을 나눈 의미가 없고, ②는 "어디"가 아니라 "무엇으로 이루어져 있나"다.
  - **③ 추진 현황**(공간 축) = 동별 진행률·잔량 · 판정 분포 · 못 한 이유 · 재방문 대기. **자체 기간 필터를 가진다.** 재방문 대기만 기간을 받지 않는다 — "지금 얼마나 밀려 있나"라 축이 다르고, 화면이 거기에 기간 라벨을 붙이면 거짓말이 된다.
  - **④ 소요 물량**(자재 축) = 예상 소요 총량 · 동별 소요 배부표 · 집행 실적 · 자재 구분(기기/전지). **자체 기간 필터.** 이름과 라벨에 "예산"을 쓰지 않는다 — 금액·재고 축이 스키마에 없어 없는 것을 약속하게 된다. `buildings.rx_code_cd`가 전 행 `RX-IOT`라 "처방 구성"이 아니라 **"예상 소요"라는 총량 표현**으로만 쓴다.
  - **⑤ 실적 통계**(시간 축) = 보고 3숫자(점검 세대 · 교체 대수 · 거부 건수) + 방문 결과 내역 + `[이 기간 기록 열람 →]`. **자체 기간 필터.** **건 단위 표를 만들지 않는다** — 열람은 `/records`가 이미 하는 일이라 여기서 또 만들면 없애려던 중복을 새로 만든다. 링크는 `/records?from=&to=`로 기간을 넘긴다.
...
코어 10종은 전부 **shadcn/ui(Radix) 프리미티브 위의 도메인 래퍼**다(`BarRow`·`StackedBar`만 대응 프리미티브가 없어 토큰 조합으로 만든다 — 인터랙션이 없는 표시 전용이라 위임할 접근성 동작이 없다). focus trap·키보드 내비게이션·ARIA를 직접 구현하지 않는다. `variant`/`size`는 유니언 타입 열거로 제한하고, 도메인 열거값(상태·처방·위험구간)은 `src/config/domain.ts`에서 주입한다. `className` 관통은 레이아웃 여백 조정용으로만 — 색·폰트 오버라이드 금지.
...
9. **`BarRow`** (`{components.bar-row}`) — 분포 1줄(라벨 + 막대 + 수치). 색은 § 분포 막대 색 규칙을 따른다. 라벨·수치 병기 필수형 — 막대만 렌더하는 사용은 막는다.
10. **`StackedBar`** (`{components.stacked-bar}`) — 100% 비율 막대. 세그먼트는 `risk-*` 톤 슬롯으로 받고 범례를 함께 낸다. **0건이 아닌 세그먼트는 `min-width: 1px`로 반드시 보인다** — 소수 구간(예: `NO_POP` 2행)이 반올림으로 사라지면 합이 100%인데 눈에는 안 보이는 상태가 된다.
...
| 폭 | 대상 | 요구 수준 |
|---|---|---|
| ≥1280px | `/control` | 여유 폭 활용(① 동별 표 확장, ②·③·④ 카드 2열) — 검증 기준은 아래 1024와 공통 |
...
- ~~도시 격자 위험 밀집 표시 방식 결정 대기~~ → **확정·구현됨(2026-08-11)**: **아주 옅은 코로플레스**를 채택했다(중심 색 도트·리스트 전용은 기각). 규칙은 § Risk & Status의 「격자 위험 코로플레스」, 구현은 `src/lib/grid-layer.ts`.
...
- **관제 ⑤ → `/records` 기간 전달의 다중 월 거동 미정**: 링크는 `/records?from=&to=`로 나가고 `/records`는 **`from`이 속한 월로 달력을 연다**. 기간이 여러 달이면 첫 달만 열린다 — 기간 전체를 표로 펼치려면 `/records`의 달력→날짜→표 구조 자체를 바꿔야 해서 보류했다. 실무에서 분기 보고가 걸리면 그때 연다.
