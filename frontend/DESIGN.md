...
- **현재 위치·거리순 정렬은 지도 실장 전까지 비활성**: 거리값의 원천이 단말 위치뿐이라 mock 값을 두지 않는다 — 정렬 버튼은 `disabled`, '현재 위치로 이동'은 no-op.
- 지도 핀의 정확한 형태·크기 스펙(줌 레벨별)은 **Naver Maps 실장 시점**에 확정 — 색·라벨 병기 규칙은 본 문서를 따른다. 함께 정할 것: **핀 렌더링 상한**(격자 하나에 최대 975건, 관악구 전체 22,501건이라 전량 개별 마커는 불가).
- 인쇄(점검 결과 출력) 스타일 미정의.
