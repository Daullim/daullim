...
colors: # [semantic 계층] — 컴포넌트·화면이 참조하는 유일한 계층. palette만 참조.
  ...
  bg-dark: "{palette.gray-900}"         # 유일한 다크 서피스 — 관제 하단 요약 바 한 곳(4탭 공통)
...
- **Dark** (`{colors.bg-dark}`): **유일한 다크 서피스** — `/control` 하단 요약 카운터 바 한 곳에만 허용. 관제 4탭이 이 바 하나를 공유하며 **KPI를 말하는 자리도 여기뿐이다**(탭별 KPI 스트립 금지). 다른 어디에도 다크 면을 만들지 않는다.
...
관제 4탭은 제목 위계를 **세 단으로 고정**한다. 탭마다 다르게 고르지 않는다.
...
- **`/control` (태블릿 1024×768 가로 = 검증 기준, ≥1280은 여유 폭 활용) — 분석형 대시보드 4탭.** 관제는 **개별 건물을 가리키지 않는다** — 건물을 짚는 순간 `/field`의 데스크톱판이 된다. 드릴다운의 종점은 **동**이고 그 아래는 `[현장모드로 전환]`이 받는다. 골격 = 상단바(56px) → **탭바** → `<Outlet />` → 하단 다크 요약 바(64px).
  - **탭 4종은 라우트다** — `/control`(관내 현황) · `/control/risk`(위험 분석) · `/control/progress`(추진 현황) · `/control/report`(실적 통계). 시연 중 특정 탭 URL 직행이 되고 새로고침에 살아남는다. **shadcn Tabs가 아니라 `<nav>` + `NavLink`** — 라우트 탭은 Tabs 프리미티브가 아니라 내비게이션이라 링크만으로 접근성이 끝난다(roving tabindex·`aria-selected`를 흉내 낼 일이 없다). Iteration Guide 6의 예외가 아니라 적용 대상이 아니다.
  - **탭바 = 좌 탭 / 우 `RegionSelector`(시·도 → 시·군·구 **2단**) + `[현장모드로 전환]`.** 관할은 네 탭 전체의 스코프라 탭보다 상위에 둔다 — 탭마다 필터를 반복하면 전환 시 스코프가 흔들린 것처럼 보인다. **읍·면·동 단계는 두지 않는다**(동은 표·지도에서 고른다).
  ...
  - **③ 추진 현황**(공간 축) = 구의 방문 세대·미방문 세대·교체 개수 · 동별 방문 세대·미방문 세대·교체 개수·재방문 대기 · 판정 분포 · 못 한 이유. **자체 기간 필터를 가진다.** 재방문 대기는 최신 방문 기준 현재 상태라 기간 필터를 받지 않는다. 교체 개수는 실제 교체에 사용된 경보기 개수 누적이다.
