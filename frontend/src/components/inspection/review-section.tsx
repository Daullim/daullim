 ...
 */
export function ReviewSection({ form }: { form: InspectionFormState }) {
  ...
  return (
    <FormSection>
      {missing.length > 0 ? (
        ...
        <InfoNote tone="positive">입력이 모두 끝났습니다. 아래 내용을 확인하고 제출하세요.</InfoNote>
      )}
      ...
      <p className="text-caption text-subtle">
        ※ 감지기 외관·권장연수(<DataText>{SERVICE_LIFE_YEARS.detector}</DataText>년) 기준은 배선형
        자탐설비 기준 준용
      </p>
    </FormSection>
  );
