...
import { doneCount, unitLabel, type UnitRow } from "@/lib/units";
...
export function UnitPanel({
  ...
  /** 다가구 — 전유부에 호 목록이 없어 현장에서 직접 입력 */
  onRenameUnit: (index: number, hoNm: string) => void;
  className?: string;
}) {
  /* 다가구는 공부에 호 목록이 없다 → 입력 가능, 집합·단독은 대장 값이라 읽기전용 */
  const editable = item.houseType === "multi-family";
  ...
  const selectedUnit = selected != null ? units[selected] : undefined;
  const needsHoNm = !!selectedUnit && !selectedUnit.hoNm.trim();

  return (
    <aside
      aria-label={`${item.address} 세대 목록`}
      className={cn(
    ...
    >
      ...
      <div className="flex shrink-0 items-start gap-2 border-b border-hairline p-3">
        ...
        <div className="min-w-0 flex-1 pt-1.5">
          <h2 className="truncate text-title-sm text-ink">{item.address}</h2>
            ...
            <span className="min-w-0 flex-1">
              {editable ? (
                <input
                  type="text"
                  value={unit.hoNm}
                  onChange={(e) => onRenameUnit(index, e.target.value)}
                  onFocus={() => setSelected(index)}
                  aria-label={`${index + 1}번째 세대 호수`}
                  placeholder='예: "201호"'
      ...
      <div className="shrink-0 space-y-2 border-t border-hairline p-3">
        ...
        <Button
          ...
          size="field-xl"
          className="w-full"
          disabled={selected == null || needsHoNm}
          onClick={() => selected != null && onInspect(selected)}
        >
          점검하기
        </Button>
      </div>
