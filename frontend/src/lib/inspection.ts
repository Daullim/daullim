...
/**
 * 저장된 방문 → 폼 상태. 상세 다이얼로그가 `ReviewSection`을 그대로 재사용하기 위한 역매핑이다.
 *
 * **원입력만 되돌린다.** 판정·처방은 `ReviewSection`이 이 상태에서 다시 파생하므로 서버가 저장해 둔
 * `conditionCode`를 넣지 않는다 — 같은 규칙(`ruleVersion`)이면 같은 값이 나온다. 규칙이 갈리면
 * 화면과 저장값이 달라질 수 있고, 그때는 저장값이 진실이다.
 */
export function toFormState(visit: VisitDetail): InspectionFormState {
  return {
    consent: visit.consentCd,
    respondent: visit.respondentTypeCd,
    refusalReason: visit.refusalReasonCd,
    refusalNote: visit.refusalNote ?? "",
    roomCount: visit.roomCount,
    unitLabel: unitLabel(visit),
    mfgYm: visit.mfgYm,
    mfgUnmarked: visit.mfgUnmarked,
    replaceCount: visit.replaceCount,
    // 자동 생성 항목(경과·미표기)은 폼이 입력받지 않는 것이라 되돌리지 않는다 — 파생이 다시 만든다.
    replacements: visit.replacements
      .filter((item) => !item.autoGenerated)
      .map((item) => ({
        reason: item.replaceReasonCd,
        batteryType: item.batteryTypeCd,
        flags: item.detectorFlagCds,
      })),
    extinguisherInstalled: visit.extinguisherInstalledCd,
    rxDone: visit.rxDoneCd,
    revisit: visit.revisitPlanCd,
    noRevisitNote: visit.noRevisitNote ?? "",
    note: visit.note ?? "",
  };
}
